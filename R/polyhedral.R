...
#
.create_mosek_input_polyh <- function(A,y) {
    m <- dim(A)[1]
    n <- dim(A)[2]
    ...
    mos_inp$A     <- Matrix( as.vector(Aext), nrow=m, byrow=FALSE, sparse=TRUE )
...
.update_mosek_input_polyh <- function(mos_inp,y) {
    m <- length(y)
...
rbichibarsq_polyh <- function(n, A, reduce=TRUE) {
    ...
    if (reduce) {
        ...
        for (j in 1:nn) {
            mos_inp <- .create_mosek_input_polyh(A[ ,-j],A[ ,j])
            is_superfluous[j] <- sum( mosek(mos_inp,opts)$sol$itr$xx[(nn+2):(nn+m+1)]^2 ) == sum(A[ ,j]^2)
        }
    ...
    m <- dim(A)[1]
    nn <- dim(A)[2]
    mos_inp <- .create_mosek_input_polyh(A,rep(0,m))
    out <- matrix(0,n,2)
    for (i in 1:n) {
        y <- rnorm(m)
        mos_inp <- .update_mosek_input_polyh(mos_inp,y)
        p <- sum( mosek(mos_inp,opts)$sol$itr$xx[(nn+3):(nn+m+2)]^2 )
        out[i, ] <- c(p,sum(y^2)-p)
    }
    if (!reduce)
        return(out)
