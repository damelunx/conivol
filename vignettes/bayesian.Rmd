...
and without any other assumptions can be solved analytically, but assuming
log-concavity already prohibits an analytical solution. The case of reconstructing
the intrinsic volumes based on bivariate chi-bar-squared data is even more
challenging. Therefore, the posterior distribution will be sampled through
Monte-Carlo sampling. The functions in `conivol` mostly use the sampler
...
We assume familiarity with the other two vignettes:
...
For taking the parity equation $V_0+V_2+V_4+\dots=V_1+V_3+V_5+\dots=\frac{1}{2}$
...
can be modeled through $V^e$ and $V^o$ taking values in the corresponding probability simplex:
...
$\exp(U_0)+\exp(U_2)+\exp(U_4)+\dots=\exp(U_1)+\exp(U_3)+\exp(U_5)+\dots=\frac{1}{2}$.
...
in `conivol` as it is unclear what approach will yield the best results.
...
for $v\in\Delta^d$, and zero else. The expectation of $V$ is given by
  \[ \text{E}[V] = \frac{(\alpha_0,\ldots,\alpha_d)}{\alpha_0+\dots+\alpha_d} , \]
and the *prior sample size*, cf. [@GCSDVR14], is given by $\alpha_0+\dots+\alpha_d$.
The $k$th marginal distribution is concave iff $\alpha_k\geq1$, and if $\alpha_k>1$
for all $k$, then the mode of the $k$th marginal distribution is given by
  \[ \text{mode}(V_k) = \frac{\alpha_k-1}{(\alpha_0-1)+\dots+(\alpha_d-1)} . \]
We thus arrive at two natural choices for the parameters of the prior distribution,
one noninformative, the other (slightly) informative; both using the starting
point of the EM algorithm $v^{(0)}$ (see the
[previous vignette](estim-conic-intrinsic-volumes-with-EM.html#start_EM)):

1. *noninformative:* $\alpha_k = v^{(0)}_k$ for $k=0,\ldots,d$.
    In this case we have matched the first moment: $\text{E}[V]=v^{(0)}$.
    The marginal distributions are convex and widely spread; the prior sample
    size is $\alpha_0+\dots+\alpha_d=1$.
    Note that in this case we must have $v^{(0)}_k>0$ for all $k$.
2. *informative:* $\alpha_k = 1 + v^{(0)}_k$ for $k=0,\ldots,d$.
    In this case we have matched the modes: $\text{mode}(V_k)=v^{(0)}_k$.
    The marginal distributions are concave; the prior sample size is
    $\alpha_0+\dots+\alpha_d=d+2$.
...
$\alpha^o=(\alpha^o_1,\alpha^o_3,\alpha^o_5,\ldots)$ chosen either in a
noninformative or in an informative way:

1. *noninformative:* $\alpha^e_j = 2v^{(0)}_j$, $\alpha^o_k = 2v^{(0)}_k$,
    for $j$ even and $k$ odd,
2. *informative:* $\alpha^e_j = 1 + 2v^{(0)}_j$, $\alpha^o_k = 1 + 2v^{(0)}_k$,
    for $j$ even and $k$ odd.
...
positive orthant. We use the gamma distribution,
  \[ S_k \sim \text{Gamma}(\alpha_k,\beta_k) , k=0,\ldots, \]
where $\alpha_k>0$ and $\beta_k>0$ denote the shape and inverse scale, that is,
  \[ p(S_k=\theta) \propto \frac{\theta^{\alpha_k-1}}{\exp(\beta_k\theta)} . \]
Expectation and mode are given by 
\begin{align*}
    \text{E}[S_k] & = \frac{\alpha_k}{\beta_k} ,
    & \text{mode}(S_k) & = \frac{\alpha_k-1}{\beta_k} .
\end{align*}
We choose these constants in the following way: the initial guess for the
intrinsic volumes $v^{(0)}$ yields an initial guess for the transformed variable,
$s^{(0)}=T\log(v^{(0)})$, which we then use in one of the following ways to
find the shape and inverse scale of the gamma distribution,

1. *noninformative:* $\alpha_k = 1$, $\beta_k = \frac{1}{s^{(0)}_k}$,
2. *informative:* $\alpha_k = \frac{s^{(0)}_k}{2}$, $\beta_k = \frac{1}{2}$,
