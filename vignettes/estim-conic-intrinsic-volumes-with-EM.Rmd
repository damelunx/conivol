...
and implemented in `estim_em`. We also include formulas for the gradient and
...
invariant. The sample of the bivariate chi-bar-squared distribution then becomes
...
is implemented in `loglike_ivols`. This function also returns the gradient
and Hessian of the rescaled log-likelihood function (in the affine-linear subspace
respecting the parity equations of the intrinsic volumes). See the
[appendix below](#grad-hess-loglike) for formulas for the gradient and Hessian.
...
The expectation maximization (EM) algorithm works by maximizing, with respect
to the latent variable, the conditional
expectation of the maximum likelihood method,
given the current iterate for the parameter that is to be found.
...
### Appendix A: Gradient and Hessian {#grad-hess-loglike}

Gradient descent and Newton's method both seem to be inappropriate for the task
of reconstructing the intrinsic volumes from 
bivariate chi-bar-squared
sample data as described above.
However, analyzing the gradient and the Hessian might still be instructive to
decide whether the maximum likelihood estimate has been reached.
...
For this we change coordinates in the following way:
let $k_0,k_1\in\{1,\ldots,d-1\}$,
$k_0$ even, $k_1$ odd,
arbitrary but fixed,
and parametrize the neighborhood of the vector $v$ through
  \[ v(a) := v + \sum_{k\text{ even}} a_k (e_k-e_{k_0}) + \sum_{k\text{ odd}} a_k (e_k-e_{k_1}) , \]
so that the coordinates are given by
  \[ v_k(a) = \begin{cases}
    v_{k_0} - \sum_{k\text{ even}, k\neq k_0} a_k & \text{for } k=k_0
 \\ v_{k_1} - \sum_{k\text{ odd}, k\neq k_1} a_k & \text{for } k=k_1
 \\ v_k + a_k & \text{else} .
 \end{cases} \]
Changing coordinates in $F_\lambda$ by defining
\begin{align*}
   \tilde F_\lambda(a) := F_\lambda(v(a)) & = p \log v_d(a) + q \log v_0(a)
    + \frac{1}{N}\sum_{i=1}^{\bar N} \log\Big(\sum_{k=1}^{d-1} v_k(a) f_{ik}\Big)
\\ & + \sum_{k=1}^{d-1} \lambda_k (2\log v_k(a) - \log v_{k-1}(a) - \log v_{k+1}(a)) ,
\end{align*}
we obtain the following gradient in zero:

\begin{align*}
  \nabla \tilde F_\lambda(0) & = \text{diag}(v)^{-1} \begin{pmatrix} q
    \\ 0 \\ p \end{pmatrix} + \frac{1}{N} \sum_{i=1}^{\bar N}
    \frac{f_i - f_{ik_0}\mathbf1_{\text{even}} - f_{ik_1}\mathbf1_{\text{odd}}}{ \sum_{k=1}^{d-1} v_kf_{ik} }
\\[2mm] & + \text{diag}(v)^{-1} \begin{pmatrix} -\lambda_1 \\ 2\lambda-\lambda_{-1}-\lambda_{+1}
    \\ -\lambda_{d-1} \end{pmatrix} - \frac{2\lambda_{k_0}-\lambda_{k_0-1}-\lambda_{k_0+1}}{v_{k_0}}
    \mathbf1_{\text{even}} - \frac{2\lambda_{k_1}-\lambda_{k_1-1}-\lambda_{k_1+1}}{v_{k_1}}
    \mathbf1_{\text{odd}} ,
\end{align*}
where

* $f_i = (0,f_{i1},\ldots,f_{i,d-1},0)^T$,
* $\mathbf1_{\text{even}} = (1, 0, 1, 0, 1, \ldots)^T$, $\mathbf1_{\text{odd}}
    = (0, 1, 0, 1, 0, 1, \ldots)^T$,
* $\lambda=(\lambda_1,\ldots,\lambda_{d-1})^T$, $\lambda_{-1}=(1,\lambda_1,
    \ldots,\lambda_{d-2})^T$, $\lambda_{+1}=(\lambda_2,\ldots,\lambda_{d-1},1)^T$

Note that $\tilde F_\lambda$ is constant in $a_{k_0}$ and $a_{k_1}$, and the
gradient is correspondingly zero in these points; the gradient of $F_\lambda$
projected to the affine subspace $V$ is thus (up to rescaling) given by
...
\begin{align*}
  H\tilde F_\lambda(0) & = - \text{diag}(v)^{-2}\text{diag} \begin{pmatrix} q \\
    0 \\ p \end{pmatrix}
  -\frac{1}{N} \sum_{i=1}^{\bar N}
    \frac{ \big( f_i - f_{ik_0}\mathbf1_{\text{even}} - f_{ik_1}\mathbf1_{\text{odd}} \big)
    \big( f_i - f_{ik_0}\mathbf1_{\text{even}} - f_{ik_1}\mathbf1_{\text{odd}} \big)^T}
    { \big(\sum_{k=1}^{d-1} v_kf_{ik}\big)^2 }
\\ & - \text{diag}(v)^{-2}\text{diag} \begin{pmatrix} -\lambda_1 \\
    2\lambda_j-\lambda_{j-1}-\lambda_{j+1} \\ -\lambda_{d-1} \end{pmatrix}
    - \frac{2\lambda_{k_0}-\lambda_{k_0-1}-\lambda_{k_0+1}}
    {v_{k_0}^2} \mathbf1_{\text{even}}\mathbf1_{\text{even}}^T
\\ & - \frac{2\lambda_{k_1}-\lambda_{k_1-1}-\lambda_{k_1+1}}{v_{k_1}^2}
    \mathbf1_{\text{odd}}\mathbf1_{\text{odd}}^T .
\end{align*}
As noted above for the gradient, the Hessian matrix will have rank $d-2$;
eliminating the rows and columns with indices $k_0$ and $k_2$ will give a
full rank $(d-2)\times(d-2)$ matrix which can be used for an implementation of
Newton's method. This step in the parameter space $\text{R}^{d-2}$ then has
to be translated back to $\text{R}^d$.

The function `loglike_ivols` returns gradient and Hessian in zero of $\tilde F_\lambda$
along with index vectors for the above described subsetting steps.
