...
2. *circular cones*: $d$-dimensional cones of angle $\alpha$ have an approximate statistical dimension of $d\sin^2\alpha$ and an approximate variance of $(d/2-1)\sin^2(2\alpha)$. We can thus choose $\alpha$ to match either the statistical dimension, or the variance, or take an average of both fits:
    (a) *match first moment*: take $\alpha=\text{arcsin}\sqrt{\hat\delta/d}$ and take $v^{(0)}$ to be the intrinsic volumes of a $d$-dimensional circular cone with an angle of $\alpha$.
    (b) *match second moment*: if $\hat\delta<d/2$ then take $\beta=\frac{1}{2}\text{arcsin}\sqrt{\frac{2\widehat{\text{var}}}{d-2}}$ else take $\beta=\frac{\pi}{2}-\frac{1}{2}\text{arcsin}\sqrt{\frac{2\widehat{\text{var}}}{d-2}}$ and take $v^{(0)}$ to be the intrinsic volumes of a $d$-dimensional circular cone with an angle of $\beta$.
...
\\ \iff & \text{minimize } (-A^Tz)^Tu + r \quad\text{subject to}\quad u\geq0, Au=w, s=1, \|w\|^2\leq 2rs .
\end{align*}
...
This is the approach taken in the function `polyh_rbichibarsq`.
...
The problem of finding the projection of a point $z\in\text{R}^d$ onto an ellipsoidal cone given as the (full-dimensional) linear image of the Lorentz cone, $A\mathcal{L}^d$, $A\in\text{Gl}_d$, can also be formulated as an (SOCP):

\begin{align*}
   & \underset{x\in A\mathcal{L}^d}{\text{argmin}}\, \|x-z\|_2
\\ \iff & \text{minimize } \tfrac12 \|x\|^2 - z^T x \quad\text{subject to}\quad x=Ay, y\in\mathcal{L}^d
\\ \iff & \text{minimize } (-z)^Tx + r \quad\text{subject to}\quad x=Ay, y\in\mathcal{L}^d, s=1, \|x\|^2\leq 2rs .
\end{align*}
This last formulation can be solved by MOSEK, using the rotated quadratic cone constraint.
