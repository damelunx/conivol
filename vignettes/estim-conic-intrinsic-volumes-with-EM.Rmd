...
This vignette describes the details of the algorithm to find the intrinsic volumes of closed convex cones from samples from the associated bivariate chi-bar-squared distribution. A general knowledge of the theory of conic intrinsic volumes is assumed, see [this vignette](conic-intrinsic-volumes.html) for a short introduction.
...
$$\Pi_C(z) = \text{argmin}\{\|x-z\|\mid x\in C\}.$$
  We will assume in the following that both $C$ and $C^\circ$ have nonempty interior, or equivalently, $C^\circ$ and $C$ both do not contain a nonzero linear subspace. These linear algebraic conditions are easily checked/established so there is no serious restriction in assuming this.  
...
* $v = v(C) = (v_0(C),\ldots,v_d(C))$ denotes the vector of intrinsic volumes. Under the (primal and polar) nonempty interior assumption we have $v_k>0$ for all $k=0,\ldots,d$.^[This actually requires a proof in the nonpolyhedral case (and without having log-concavity); one can deduce this from the kinematic formula by projecting the cone on a uniformly random linear subspace and looking at the volume of the (relative) boundary of the projection.]
* We work with the two main random variables
$$ X=\|\Pi_C(g)\|^2 ,\quad Y=\|\Pi_{C^°}(g)\|^2, $$
  where $g\sim N(0,I_d)$. So $X$ and $Y$ are chi-bar-squared distributed with reference cones $C$ and $C^\circ$, respectively, and the pair $(X,Y)$ distributed according to the bivariate chi-bar-squared distribution with reference cone $C$.
* For later use in the EM algorithm we also define the latent variable $Z\in\{0,1,\ldots,d\}$, $\text{Prob}\{Z=k\}=v_k$.

Concretely, if $C$ is a polyhedral cone then we define $Z$ as the dimension of the face of $C$ such that $\Pi_C(g)$ lies in its relative interior. This is well-defined, as $C$ decomposes into a disjoint union of the relative interiors of its faces. We may assume without loss of generality that the underlying cone is polyhedral, as any closed convex cone can be arbitrarily well approximated by polyhedral cones.
...
We assume to have sampled data: $(X_1,Y_1),\ldots,(X_N,Y_N)$ denote iid copies of $(X,Y)$ and we assume that they took the sample values: $(x_1,y_1),\ldots,(x_N,y_N)$. The general goal is to reconstruct $v$ from these sample values.


## Choosing a starting point

We need to find a starting point for the iterative algorithms to reconstruct $v$. This can of course be the uniform distribution on $\{0,\ldots,d\}$, but we may as well use a more elaborated first guess. The first and second moment of $v$, more precisely, of the latent variable $Z$, can be conveniently estimated from the sample data, as
$$ \delta := \sum_{k=0}^d k v_k(C) = \text{E}[X] = d-\text{E}[Y] , $$
and
$$ \text{var} := \sum_{k=0}^d (k-\delta)^2 v_k(C) = \text{E}[X^2]-(\delta+1)^2+1 = \text{E}[Y^2]-(d-\delta+1)^2+1 . $$
...
These formulas are implemented in `conivol::estimate_statdim_var`.
...
$$ v^{(0)}_k = \begin{cases} \text{Prob}\{\nu\leq 0.5\} & \text{if } k=0 , \\ \text{Prob}\{k-0.5\leq \nu\leq k+0.5\} & \text{if } 0<k<d , \\ \text{Prob}\{\nu\geq k-0.5\} & \text{if } k=d , \end{cases} $$
...
These formulas are implemented in `conivol::init_v`.


## The likelihood function

The likelihood function is straightforward to derive, but one has to be cautious about the "edge cases", i.e., the cases where $g\in C\cup C^\circ$. Notice that this event is visible in the sample data, as
$$ g\in C \iff Y=0 ,\qquad g\in C^\circ \iff X=0 . $$
Moreover, if $Y=0$, then the value of $X$ is just a draw from the $\chi_d^2$ distribution, and similarly for $X=0$. This shows that those sample values $(x_i,y_i)$ with $x_i=0$ or $y_i=0$ are just noise and should be discarded. The non-noisy part is of course the *proportion* of points with $Y=0$ and $X=0$, which form point estimates for $v_d(C)$ and $v_0(C)$, respectively. The data on which we base our estimation thus has the following form:
...
In fact, as it turns out we use the remaining sample points through the density values
$$ f_k(x_i)f_{d-k}(y_i) ,\quad k=1,\ldots,d-1,\; i=1,\ldots,\bar N , $$
where $f_k$ denotes the density of the chi-squared distribution,
$$ f_k(t)=\frac{1}{2^{k/2}\Gamma(k/2)}x^{k/2-1}e^{-x/2} . $$
The function `conivol::prepare_data` evaluates the sample data in this regard; calling this function outside the EM algorithm allows to skip this step on multiple evaluations of the EM algorithm or related functions.

We obtain the following formula for the likelihood of the given data:
$$ L(v\mid \mathbf{x},\mathbf{y}) = v_0^{|\{i\mid x_i=0\}|} v_d^{|\{i\mid y_i=0\}|} \prod_{i=1}^{\bar N}\sum_{k=1}^{d-1} v_k f_k(x_i) f_{d-k}(y_i) . $$
...
$$ L(v\mid \mathbf{x},\mathbf{y},\mathbf{z}) = v_0^{|\{i\mid x_i=0\}|} v_d^{|\{i\mid y_i=0\}|} \prod_{i=1}^{\bar N}\prod_{k=1}^{d-1} (v_k f_k(x_i) f_{d-k}(y_i))^{(z_i=k)} , $$
where $(z_i=k)=1$ if $z_i=k$ and zero else.

For numerical reasons we work with the rescaled log-likelihood functions
$$\begin{align*}
\frac{1}{N} \log L(v\mid \mathbf{x},\mathbf{y}) & = q \log v_0 + p \log v_d + \frac{1}{N}\sum_{i=1}^{\bar N} \log\Big(\sum_{k=1}^{d-1} v_k f_k(x_i) f_{d-k}(y_i)\Big) ,
\\ \frac{1}{N} \log L(v\mid \mathbf{x},\mathbf{y},\mathbf{z}) & = q \log v_0 + p \log v_d + \frac{1}{N}\sum_{i=1}^{\bar N} \sum_{k=1}^{d-1} (z_i=k) \big( \log v_k + \log f_k(x_i) + \log f_{d-k}(y_i) \big) .
\end{align*}$$
The rescaled log-likelihood function $\frac{1}{N} \log L(v\mid \mathbf{x},\mathbf{y})$ is implemented in `conivol::comp_loglike`.
...
The validity of these inequalities is an open conjecture for general cones at this moment. But there is strong evidence that they hold in general, and assuming their validity greatly helps the EM algorithm to converge to a sensible estimate.

There are two natural ways in which log-concavity may be exploited:
...
$$ G_\lambda(v,\mathbf{z}) := \frac{1}{N} \log L(v\mid \mathbf{x},\mathbf{y},\mathbf{z}) + \sum_{k=1}^{d-1} \lambda_k (2\log v_k - \log v_{k-1} - \log v_{k+1}) , $$
   where $\lambda_1,\ldots,\lambda_{d-1}\geq0$ and $\lambda_0:=\lambda_d:=0$ (setting all $\lambda_k$ to zero eliminates this restriction).
...
$$ \big\{ u\in\text{R}^{d+1}\mid 2 u_k - u_{k-1} - u_{k+1} \geq c_k \big\} . $$
   for some $c_1,\ldots,c_{d-1} (\geq0)$ (letting all $c_k$ go to $-\infty$ eliminates this restriction).

Both of these methods are supported in `conivol::find_ivols_EM`.

### Gradient and Hessian

Gradient descent and Newton's method both seem to be inappropriate for the task of reconstructing the intrinsic volumes from sample data as described above. However, analyzing the gradient and the Hessian might still be instructive to decide whether the maximum likelihood estimate has been achieved.

We compute the gradient and Hessian in the the affine-linear subspace $\{v\in\text{R}^{d+1}\mid v_0 + v_2 + \dots = v_1 + v_3 + \dots = \frac12\}$. The negative of the gradient of the penalized log-likelihood function in $\mathbf x$ and $\mathbf y$ is given by
$$\begin{align}
  -\nabla F_\lambda(v) & = \frac{1}{N} \sum_{i=1}^{\bar N} \frac{ \text{diag}(v) \Bigg(f_{ik_0}\mathbf1_{\text{even}} + f_{ik_1}\mathbf1_{\text{odd}} - \begin{pmatrix} 0 \\ f_i \\ 0 \end{pmatrix} \Bigg) }{ \sum_{k=1}^{d-1} v_kf_{ik} } -\begin{pmatrix} q-\lambda_1 \\ 2\lambda-\lambda_{-1}-\lambda_{+1} \\ p-\lambda_{d-1} \end{pmatrix}
\\[2mm] & \quad + (2\lambda_{k_0}-\lambda_{k_0-1}-\lambda_{k_0+1})\frac{\text{diag}(v) \mathbf1_{\text{even}}}{v_{k_0}} + (2\lambda_{k_1}-\lambda_{k_1-1}-\lambda_{k_1+1})\frac{\text{diag}(v)\mathbf1_{\text{odd}}}{v_{k_1}} ,
\end{align}$$
where

* $f_{ik}=f_k(x_i)f_{d-k}(y_i)$, $f_i = (f_{i1},\ldots,f_{i,d-1})^T$,
...
$$\begin{align}
  -HF_\lambda(v) & = \frac{1}{N} \sum_{i=1}^{\bar N} \frac{ \text{diag}(v) \Bigg(f_{ik_0}\mathbf1_{\text{even}} + f_{ik_1}\mathbf1_{\text{odd}} - \begin{pmatrix} 0 \\ f_i \\ 0 \end{pmatrix} \Bigg) \Bigg(f_{ik_0}\mathbf1_{\text{even}} + f_{ik_1}\mathbf1_{\text{odd}} - \begin{pmatrix} 0 \\ f_i \\ 0 \end{pmatrix} \Bigg)^T \text{diag}(v)}{ \big(\sum_{k=1}^{d-1} v_kf_{ik}\big)^2 }
\\ & \qquad + \text{diag} \begin{pmatrix} p-\lambda_1 \\ 2\lambda_j-\lambda_{j-1}-\lambda_{j+1} \\ q-\lambda_{d-1} \end{pmatrix} + (2\lambda_{k_0}-\lambda_{k_0-1}-\lambda_{k_0+1})\text{diag}(\tfrac{v}{v_{k_0}}) \mathbf1_{\text{even}}\mathbf1_{\text{even}}^T\text{diag}(\tfrac{v}{v_{k_0}})
\\ & \qquad + (2\lambda_{k_1}-\lambda_{k_1-1}-\lambda_{k_1+1})\text{diag}(\tfrac{v}{v_{k_1}}) \mathbf1_{\text{odd}}\mathbf1_{\text{odd}}^T\text{diag}(\tfrac{v}{v_{k_1}})
\end{align}$$

These formulas are implemented in `conivol::find_ivols_gradient` and `conivol::find_ivols_Hessian`.(TBD)

## Expectation maximization (EM) algorithm

The expectation maximization (EM) algorithm works by maximizing the conditional expectation with respect to the latent variable of the maximum likelihood method, given the current iterate for the parameter that is to be found. In the situation under review, we seek to maximize the function
$$\begin{align}
...
## Addendum: SOCP formulation of projection maps and MOSEK implementation
...
This is the approach taken in the function `conivol::rbichibarsq_polyh`. (STILL NEED TO COMPLETE DIMENSION REDUCTION THERE..)
...
\end{align*}
Again, this last formulation is solved with MOSEK, using the rotated quadratic cone constraint, to enforce the log-concavity constraints.
