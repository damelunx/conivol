...
In the following two subsections we describe how this is achieved and implemented in `rbichibarsq_polyh_gen` and `rbichibarsq_polyh_ineq`, depending on whether the cone is given by generators or inequalities.
...
This simple algorithm is described in [@WW67]; there are more efficient algorithms, see [@DL06] and the references therein, but in small and medium sized dimensions the simple algorithm should be sufficient. This algorithm is implemented in `rbichibarsq_polyh_gen`, it produces a list
...
We use polarity to reduce this case to the case of cones given by generators. Calling `rbichibarsq_polyh_gen` with input matrix $A$ produces the list
$$ [ \text{dim}(C^\circ),\text{lin}(C^\circ),Q_L,Q_C,\tilde A,M'] . $$
We compute the corresponding out of `rbichibarsq_polyh_ineq`,
$$ [ \text{dim}(C),\text{lin}(C),Q_L,Q_C,\tilde A,M ] , $$
