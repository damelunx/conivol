...
$\delta(C)=\delta(\text{Circ}_d(\beta))$ then
...
What *is* known at this point is the inequality
  \[ \text{var}(C) \leq 2\text{min}\{\delta(C),d-\delta(C)\} , \]
which was shown in [@McCT14]. A proof of the above conjecture would greatly
improve this bound; see the example computations below.
...
```{r statdim-var-circ, fig.width = 7}
d <- 9
N <- 1e3
alpha <- (0:N)/N * pi/2
...
```{r statdim-var-prod-curv, fig.width = 7}
...
```{r statdim-var-prod-scatter, fig.width = 7, fig.height=10, echo=FALSE}
