...
The first and second moment of the intrinsic volumes are given by `r round(sum((0:d) * v), 4)`
and `r round(sum((0:d)^2 * v)-sum((0:d) * v)^2, 4)`, and estimated from the sample data
as `r round(estimate_statdim_var(d,m_samp)$delta, 4)` and `r round(estimate_statdim_var(d,m_samp)$var, 4)`,
...
tib_plot1 <- as_tibble( t(EM1$iterates[1+20*(0:10), ]) ) %>%
    add_column(k=0:d,.before=1) %>%
    gather(step,value,2:12)
ggplot(tib_plot1,aes(x=k,y=value,color=step)) +
    ...
    theme(legend.position="none",
          axis.title.x=element_blank(),
          axis.title.y=element_blank())
# plotting some iterates of initial mode == 4
tib_plot4 <- as_tibble( t(EM4$iterates[1+20*(0:10), ]) ) %>%
    add_column(k=0:d,.before=1) %>%
    gather(step,value,2:12)
...
```{r echo=FALSE, fig.align='center'}
tib_exact_log <- tibble( k=0:d, exact=log(v))
# mode==0
tib_plot_log0 <- as_tibble( t(log(EM0$iterates[1+20*(0:10), ])) ) %>%
    add_column(k=0:d,.before=1) %>%
    gather(step,value,2:12)
ggplot(tib_plot_log0,aes(x=k,y=value,color=step)) +
    geom_line() +
    geom_line(data=tib_exact_log,aes(x=k,y=exact),colour="black",linetype="dashed") +
    theme_bw() +
    theme(legend.position="none",
          axis.title.x=element_blank(),
          axis.title.y=element_blank())
# mode==1
tib_plot_log1 <- as_tibble( t(log(EM1$iterates[1+20*(0:10), ])) ) %>%
    add_column(k=0:d,.before=1) %>%
    gather(step,value,2:12)
ggplot(tib_plot_log1,aes(x=k,y=value,color=step)) +
    geom_line() +
    geom_line(data=tib_exact_log,aes(x=k,y=exact),colour="black",linetype="dashed") +
    theme_bw() +
    theme(legend.position="none",
          ...
          axis.title.y=element_blank())
# plotting some iterates of initial mode == 4
tib_plot_log4 <- as_tibble( t(log(EM4$iterates[1+20*(0:10), ])) ) %>%
    add_column(k=0:d,.before=1) %>%
    gather(step,value,2:12)
ggplot(tib_plot_log4,aes(x=k,y=value,color=step)) +
    geom_line() +
    geom_line(data=tib_exact_log,aes(x=k,y=exact),colour="black",linetype="dashed") +
    theme_bw() +
    theme(legend.position="none",
...
ggplot(tib_plot,aes(x=k,y=value,color=step)) +
    ...
    theme(legend.position="none",
          axis.title.x=element_blank(),
          axis.title.y=element_blank())
tib_plot_log <- as_tibble( t(log(EM$iterates[1+20*(0:10), ])) ) %>%
    add_column(k=0:d,.before=1) %>%
    gather(step,value,2:12)
